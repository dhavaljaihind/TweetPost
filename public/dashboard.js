...
if (file.type === "video/mp4") {
  mediaPreviewInner.innerHTML = `
      <div class="preview-media-card preview-media-card--admin">
        <video src="${fileURL}" class="preview-media-el preview-media-el--video" controls preload="metadata"></video>
      </div>
    `;
} else {
  mediaPreviewInner.innerHTML = `
      <div class="preview-media-card preview-media-card--admin">
        <img src="${fileURL}" alt="Preview" class="preview-media-el preview-media-el--image">
      </div>
    `;
}
...
async function loadLeadersDashboard() {
  try {
    ...
    if (!Array.isArray(leaders) || leaders.length === 0) {
      container.innerHTML = "<p style='margin-top:10px;'>No leaders added.</p>";
      return;
    }
    ...
    leaders.forEach((l, index) => {
      container.innerHTML += `
        <div class="leader-row">
          <span>${l}</span>
          <button type="button" onclick="deleteLeader(${index})">Delete</button>
        </div>
      `;
    });
  } catch (e) {
