...
app.post("/admin/login", (req, res) => {
  ...
  const admin = admins.find(
    (a) => normalizeUsername(a.username) === username && String(a.password) === password
  );
...
app.post("/admin/addAdmin", isAdminAuth, isSuperAdmin, (req, res) => {
...
app.delete("/admin/deleteAdmin/:username", isAdminAuth, isSuperAdmin, (req, res) => {
...
app.post("/admin/addMaster", isAdminAuth, isSuperAdmin, (req, res) => {
...
app.delete("/admin/deleteMaster/:level/:id", isAdminAuth, isSuperAdmin, (req, res) => {
...
app.post("/admin/updateMaster", isAdminAuth, isSuperAdmin, (req, res) => {
  try {
    const level = String(req.body.level || "").trim();
...
app.post("/admin/addMasterItem", isAdminAuth, isSuperAdmin, (req, res) => {
...
app.delete("/admin/deleteMasterItem", isAdminAuth, isSuperAdmin, (req, res) => {
  const key = String(req.body.key || "").trim();
  const value = normalizeMasterName(req.body.value);
...
/* ================= USER SIGNUP ================= */
/* ================= USER SIGNUP ================= */
app.post("/user/request-signup-otp", (req, res) => {
  try {
...
if (type === "email") {
  ...
  if (exists) {
    ...
    if (type === "mobile" && users.find((u) => u.mobile === value)) {
      return res.status(409).json({ success: false, message: "Mobile already registered" });
    }
    if (type === "email" && users.find((u) => normalizeEmail(u.email) === value)) {
      return res.status(409).json({ success: false, message: "Email already registered" });
    }

    if (DEMO_MODE) {
      return res.json({ success: true, message: `Demo: any ${type} OTP works` });
    }
  ...
  expiresAt
    ...
    console.log(`[OTP][SIGNUP] ${type}: ${value} OTP: ${otp} (expires 5 min)`);
    return res.json({ success: true, message: `${type === "email" ? "Email" : "Mobile"} OTP sent` });
  } catch (e) {
    console.error(e);
...
app.post("/user/verify-signup-otp", (req, res) => {
  try {
    const type = String(req.body.type || "").trim().toLowerCase();
...
/* ================= FORGOT PASSWORD ================= */
app.post("/user/request-otp", (req, res) => {
  try {
    ...
    console.log(`[OTP][RESET] ${parsed.type}: ${parsed.value} OTP: ${otp} (expires 5 min)`);
...
app.post("/user/reset-password", async (req, res) => {
  try {
    ...
    if (!rec) return res.status(401).json({ success: false, message: "Wrong OTP" });
    if (Date.now() > rec.expiresAt)
      return res.status(401).json({ success: false, message: "OTP expired" });
...
app.post("/admin/saveTrendSettings", isAdminAuth, (req, res) => {
  try {
    writeTrendSettings({
...
app.delete("/admin/deleteUser/:userid", isAdminAuth, (req, res) => {
  if (req.session.adminRole !== "super") return res.status(403).json({ success: false });
...
app.post("/admin/toggleBlock/:userid", isAdminAuth, (req, res) => {
  if (req.session.adminRole !== "super") return res.status(403).json({ success: false });
